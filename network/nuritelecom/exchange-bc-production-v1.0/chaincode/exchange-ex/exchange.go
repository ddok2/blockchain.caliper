...
func (t *SmartContract) batchRegisterMember(stub shim.ChaincodeStubInterface, args []string) string {
	...
	CurrencyCode(%s), MemberRole(%s), Wallet Address(%s), txTime(%s)\n`,
...
func (t *SmartContract) issueCoin(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if len(args) != 4 {
		logger.Error("issueCoin Incorrect arguments. Expecting 4 arguments")
		ret = BadRequest
		return t.doResult(ret)
	}
	logger.Infof(`issueCoin Args: TxID(%s), WalletAddress(%s), Amount(%s), txTime(%s)\n`, args[0], args[1], args[2], args[3])
