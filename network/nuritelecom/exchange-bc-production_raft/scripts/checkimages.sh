...
function checkImages() {
  ...
  if [[ "$(docker images -q hyperledger/fabric-ca:${HF_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
    echo "hyperledger/fabric-ca - invalid image version"
    exit 1
  fi

  if [[ "$(docker images -q hyperledger/fabric-zookeeper:${HF_TOOL_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
    echo "hyperledger/fabric-zookeeper - invalid image version"
    exit 1
  fi

  if [[ "$(docker images -q hyperledger/fabric-kafka:${HF_TOOL_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
    echo "hyperledger/fabric-kafka - invalid image version"
    exit 1
  fi

  if [[ "$(docker images -q hyperledger/fabric-orderer:${HF_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
    echo "hyperledger/fabric-orderer - invalid image version"
    exit 1
  fi

  if [[ "$(docker images -q hyperledger/fabric-peer:${HF_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
    echo "hyperledger/fabric-peer - invalid image version"
    exit 1
  fi

  if [[ "$(docker images -q hyperledger/fabric-tools:${HF_IMAGE_VERSION} 2> /dev/null)" == "" ]]; then
