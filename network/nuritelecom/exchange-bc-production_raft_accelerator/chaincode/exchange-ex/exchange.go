...
func (t *SmartContract) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	switch fnc {
	case "registerMember":
		return invoke(stub, t.registerMember)
	}
	return shim.Error("asdfljkasdjfklasdfkl")
}
...
func (t *SmartContract) registerMember(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if len(args) != RegisterMemberArgs {
		logger.Errorf("registerMember Incorrect arguments. Expecting %s arguments", RegisterMemberArgs)
		// ret = BadRequest
	...
	if memberBytes != nil {
		return shim.Error("AlreadyRegistered: " + string(memberBytes))
	}
