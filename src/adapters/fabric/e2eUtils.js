...
function init(config_path) {
  ...
  isLegacy = (config.info.Version.startsWith('1.0') ||
      config.info.Version.startsWith('1.1'));
  if (!isLegacy) {
...
async function installChaincode(org, chaincode) {
  ...
  the_user = await testUtil.getSubmitter(client, true /* get peer org admin */,
      org);
  ...
  let metadataPath = chaincode.metadataPath ? commUtils.resolvePath(
      chaincode.metadataPath) : chaincode.metadataPath;
    ...
    let txPromise = new Promise((resolve, reject) => {
      ...
      eh.registerTxEvent(deployId.toString(), (tx, code) => {
        commLogger.info('The chaincode ' + type +
            ' transaction has been committed on peer ' + eh.getPeerAddr());
        clearTimeout(handle);
        if (code !== 'VALID') {
          commLogger.warn(
              'The chaincode ' + type + ' transaction was invalid, code = ' +
              code);
          reject();
        } else {
...
async function instantiateLegacy(chaincode, endorsement_policy, upgrade) {
  ...
  data = fs.readFileSync(
      commUtils.resolvePath(ORGS[userOrg][eventPeer].tls_cacerts));
  ...
  try {
    ...
    for (let i in proposalResponses) {
      commLogger.debug('instantiateChaincode responses: i=' + i + ' ' +
          JSON.stringify(proposalResponses[i]));
      let one_good = false;
    ...
    if (!all_good) {
      throw new Error('Failed to send ' + type +
          ' Proposal or receive valid response. Response null or status is not 200.');
    ...
    try {
      ...
      commLogger.error('Failed to send ' + type +
          ' transaction and get notifications within the timeout period.');
      throw err;
    }
        ...
        eh.registerTxEvent(txId,
            (tx, code) => {
              ...
              if (code !== 'VALID') {
                ...
                invokeStatus.SetErrMsg(TxErrorIndex.BadEventNotificationError,
                    err.toString());
            ...
            (err) => {
              ...
              invokeStatus.SetFlag(errFlag);
              invokeStatus.SetErrMsg(TxErrorIndex.EventNotificationError,
                  err.toString());
              resolve();
            },
        );
    ...
    try {
      ...
      if (signedProposal === null) {
        if (txFile && txFile.readWrite === 'write') {
          ...
          let signedTransaction = signedOffline.generateSignedTransaction(
              transactionRequest, channel);
...
async function invokebycontext(context, id, version, args, timeout) {
  ...
  if (txFile && txFile.readWrite === 'read' && signedTransactionArray.length !==
      0) {
    ...
    let errFlag = TxErrorEnum.NoError;
    invokeStatus.SetFlag(errFlag);
    return sendTransaction(context, signedTransaction, invokeStatus, startTime,
        timeout);
  }
  ...
  try {
    try {
      ...
      let proposalResponseObjectPromise = channel.sendTransactionProposal(
          proposalRequest, timeout * 1000);
      ...
      invokeStatus.SetFlag(errFlag);
      invokeStatus.SetErrMsg(TxErrorIndex.ProposalResponseError,
          err.toString());
    ...
    for (const i in proposalResponses) {
      ...
      if (proposal_response.response && proposal_response.response.status ===
        ...
        let err = new Error(
            'Endorsement denied: ' + proposal_response.toString());
        errFlag |= TxErrorEnum.BadProposalResponseError;
        invokeStatus.SetFlag(errFlag);
        invokeStatus.SetErrMsg(TxErrorIndex.BadProposalResponseError,
            err.toString());
    ...
    invokeStatus = sendTransaction(context, {
      txId: txId,
      signedTransaction: null,
      transactionRequest: transactionRequest,
    }, invokeStatus, startTime, timeout);
  } catch (err) {
    // at this point the Tx should be verified
    invokeStatus.SetStatusFail();
    commLogger.error(
...
async function createInMemoryWallet(org) {
  ...
  if (ORGS.orderer.url.startsWith('grpcs')) {
    ...
    await inMemoryWallet.import('tlsId',
        X509WalletMixin.createIdentity(org, tlsInfo.certificate, tlsInfo.key));
  }
...
async function evaluateTransaction(context, args) {
  ...
  try {
    ...
    commLogger.error(
        'failed to evaluate transaction using args [' + JSON.stringify(args) +
        '], with error: ' + (err instanceof Error ? err.stack : err));
    invokeStatus.SetStatusFail();
    invokeStatus.result = [];
    return Promise.resolve(invokeStatus);
